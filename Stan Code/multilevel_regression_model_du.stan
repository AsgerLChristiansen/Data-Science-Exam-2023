...
model {
  ...
  // Difference priors
  delta_alpha ~ normal(0.5, 1);
  delta_beta ~ normal(1., 1);
  ...
  // compute likelihood for both groups
  for (i in 1:states_1){
...
generated quantities {
   ...
   mu_alpha_prior = normal_rng(0.5, 0.2);
   mu_beta_prior = normal_rng(1., 0.5);
   delta_alpha_prior = normal_rng(0.5, 1);
   delta_beta_prior = normal_rng(1, 1);
